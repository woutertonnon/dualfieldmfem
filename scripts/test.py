...
u1 = sp.sin(2.*sp.pi*x0)*sp.cos(2.*sp.pi*x1)#-2*x0**2*(x0-1)**2 * x1*(2*x1-1)*(x1-1) * x2*(1-x2)
u2 = -sp.cos(2.*sp.pi*x0)*sp.sin(2.*sp.pi*x1)# 2*x1**2*(x1-1)**2 * x0*(2*x0-1)*(x0-1) * x2*(1-x2)
u3 = sp.Float(0.)
...
if True:
    ...
    testSimulationHelper.run_convergence(1.,0.02,[0,1,2],[1,2])
    #testSimulationHelper.generate_config_files(1.,0.02,[0,1,2,3,4,5],[1,2],tol=1e-5)
